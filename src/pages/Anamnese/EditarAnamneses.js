// Aqui está o esqueleto base de uma tela de edição reutilizando sua lógica e estrutura
// Altera apenas o comportamento para EDITAR ao invés de criar uma nova anamnese
...
const EditarAnamnese = () => {
  ...
  useEffect(() => {
    const fetchAnamnese = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/anamnese/${id}`, {
        ...
        mostrarErro("Erro", "Falha ao carregar os dados da anamnese.");
        setLoading(false);
      }
    };
  ...
  const handleUpdate = async (updatedData) => {
    try {
      const response = await fetch(`http://localhost:5000/api/anamnese/${id}`, {
        ...
        body: JSON.stringify({ ...updatedData, executado_por: user.id }),
      });

      if (response.ok) {
        mostrarSucesso("Sucesso", "Anamnese atualizada com sucesso!");
        navigate("/home");
      } else {
        const err = await response.json();
        mostrarErro("Erro", err.message || "Erro ao atualizar.");
      }
    } catch (error) {
      console.error("Erro ao atualizar:", error);
      mostrarErro("Erro", "Falha na comunicação com o servidor.");
    }
  };

  if (loading) return <div>Carregando...</div>;
  if (!formData) return <div>Anamnese não encontrada.</div>;
